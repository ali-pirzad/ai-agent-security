...
resource "azapi_resource" "ai_foundry" {
  ...
  body = {
    ...
    properties = {
      ...
      # Reachable from one allow-listed workstation IP AND from the private
      # endpoint. For a fully-private (jump-box-only) posture, set
      # publicNetworkAccess = "Disabled" and remove the ipRules below.
      publicNetworkAccess = "Enabled"
      networkAcls = {
        defaultAction = "Deny"
        ipRules = [
          {
            value = var.allowed_client_ip
          }
        ]
      }
...
## Create a chat model deployment for the agent in the AI Foundry resource
## Note: gpt-4o is retired for new deployments; using a current GA gpt-5 model.
