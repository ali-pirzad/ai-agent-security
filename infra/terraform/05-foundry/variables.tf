...
variable "allowed_client_ip" {
  description = "Public IP address (single workstation) allowed to reach the Foundry account through the account firewall. Set publicNetworkAccess to \"Disabled\" and remove ipRules in main.tf for a fully-private, jump-box-only posture. Provide via terraform.tfvars (not committed)."
  type        = string
}
